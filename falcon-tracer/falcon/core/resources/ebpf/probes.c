...
/**
 * Probe "sys_clone" at the entry point.
 */
int entry__sys_clone(struct pt_regs *ctx)
{
    u32 kpid = bpf_get_current_pid_tgid();
...
int exit__sys_clone(struct pt_regs *ctx)
{
    ...
    u64 * timestamp = entry_timestamps.lookup(&kpid);
    if (timestamp != NULL && new_tid > 0) {
        emit_process_create(ctx, *timestamp, new_tid);
    }

    entry_timestamps.delete(&kpid);
...
int entry__sys_exit(struct pt_regs *ctx)
{
    ...
    emit_process_end(ctx, bpf_ktime_get_ns(), kpid);
